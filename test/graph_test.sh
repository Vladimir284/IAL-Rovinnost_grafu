...
do
  ...
  ./ial22 algorithm <"input/${filename_array[j]}.graph" >"${filename_array[j]}_current.output"
  diff -s -U 5 "${filename_array[j]}_current.output" "test/graph/${filename_array[j]}_reference.output"
